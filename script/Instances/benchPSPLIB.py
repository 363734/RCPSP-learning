...
from script.parameters import DIR_DATA_PREPROCESSED, DIR_DATAS, UNSEEN_SERIES, UNSEEN_WITHIN_SERIES, DIR_SPLIT
...
def split_cross_one_psplib(name: str, precfileopt, tag, u_or_b):
    step_log("Generate bench split for instance {}".format(name))
    t = from_bench(name)
    file_inst = os.path.join(DIR_DATAS, "{}/{}/{}.sm".format(PSPLIB, t, name))
    prec_file = os.path.join(DIR_DATA_PREPROCESSED, "{}/{}/{}_{}.txt".format(PSPLIB, t, name, precfileopt))
    dir_name = os.path.join(DIR_SPLIT, PSPLIB, tag)
...
def split_instances_cross_psplib(tag: str, precfileopt: str, u_or_b: str):
    for t in PSPLIB_BENCH:
        for i in range(1, PSPLIB_BENCH_GROUP[t] + 1):
            for j in range(1, 11):
                name = "{}{}_{}".format(t, i, j)
                split_cross_one_psplib(name, precfileopt, tag, u_or_b)
